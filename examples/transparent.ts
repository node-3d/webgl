...
const translateVec: [number, number, number] = [0.0, 0.0, -5.0];
const rotate100: [number, number, number] = [1, 0, 0];
const rotate010: [number, number, number] = [0, 1, 0];

const currentlyPressedKeys: Record<string, boolean> = {};

const handleKeys = (): void => {
	if (currentlyPressedKeys.BracketRight) {
		// ]
		translateVec[2] -= 0.5;
	}
	if (currentlyPressedKeys.Backslash) {
		// \
		translateVec[2] += 0.5;
	}
