...
static inline bool isArrayName(const char *name, GLsizei nameLen) {
	...
	return (name[cmpIdx + 0] == '[' && name[cmpIdx + 1] == '0' && name[cmpIdx + 2] == ']');
}
...
DBG_EXPORT JS_METHOD(getUniform) {
	...
	for (GLint i = 0; i < activeUniforms; i++) {
		...
		GLenum typeUniform;
		GLchar nameBuf[200];
		glGetActiveUniform(program, i, 200, &nameLen, &size, &typeUniform, nameBuf);
	...
	switch (pname) {
		...
		case GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES: {
			...
			glGetActiveUniformBlockiv(
			    program, uniformBlockIndex, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, &uniformCount
			);
