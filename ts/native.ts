...
type TNativeFunctions = Readonly<{
	...
	bindBufferRange: (
		target: number,
		index: number,
		buffer: TMaybeNativeResource,
		offset: number,
		size: number,
	) => void;
	bufferData: ((
		target: number,
		data: TNativeArrayData,
		usage: number,
		srcOffset?: number,
		length?: number,
	) => void) &
	...
	framebufferRenderbuffer: (
		...
		renderbuffer?: TMaybeNativeResource,
	) => void;
	framebufferTexture2D: (
		target: number,
		attachment: number,
		textarget: number,
		texture: TMaybeNativeResource,
		level: number,
	) => void;
	...
	getUniform: (
		program?: TMaybeNativeResource,
		location?: TMaybeNativeResource,
	) => TNativeUniformValue;
